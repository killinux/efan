...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="add" href="app/add" target="navTab" mask="true" rel="addApp"><span>新增商品</span></a></li>
		</ul>
	</div>
					...
					<td>
						<c:choose>
							<c:when test="${item.pubStatus == 'P' }">
								...
								<a class="btnDel" href="app/unpublish/${item.id}" target="ajaxTodo" title="下架商品">下架</a>
							</c:when>
							<c:when test="${item.pubStatus == 'T' }">
								<a class="btnEdit" href="app/update/${item.id}" target="navtab" title="编辑">编辑</a>
								<a class="btnSelect" href="app/publish/${item.id}" target="ajaxTodo" title="发布商品">发布</a>
								<a class="btnDel" href="app/unpublish/${item.id}" target="ajaxTodo" title="下架商品">下架</a>
							</c:when>
						</c:choose>
					</td>
